...
class Gs_Fit:
    ...
    def main_bs(self, data_dic):
        ...
        ra_t = []
        ra_tau = []
        for tseq, tau in itertools.product(
            range(self.ra_tmin, self.ra_tmax),
            range(1 + self.tau_cut, tseq - self.tau_cut),
        ):
            # * because the tau in the data dic is from 1 to tseq - 1 without tseq, so tau_cut = 0 means tau from 1 to tseq - 1
            ra_t.append(tseq)
            ra_tau.append(tau)
        ...
        for bs_idx in tqdm(range(N_bs)):
            ...
            for tseq in range(self.ra_tmin, self.ra_tmax):
                for tau in range(1 + self.tau_cut, tseq - self.tau_cut):
                    # * because the tau in the data dic is from 1 to tseq - 1 without tseq, so tau_cut = 0 means tau from 1 to tseq - 1
                    tau_idx = (
                        tau - 1
                    )  #! this because the tau=0 and tau=tseq in the data dic has already been cut

                    ra_re.append(
                        data_dic_gv["ra_re_tseq_{}".format(tseq)][bs_idx, tau_idx]
                    )
                    ra_im.append(
                        data_dic_gv["ra_im_tseq_{}".format(tseq)][bs_idx, tau_idx]
                    )
            ...
            if fit_res.Q < 0.05:
                tqdm.write(
                    ">>> Warning: bad fit for bs_idx = {} in fit {}".format(
                        bs_idx, self.fit_id
                    )
                )
                tqdm.write(">>> Q = {}\n".format(fit_res.Q))
            ...
            pdf_re_ls.append(
                fit_res.p["pdf_re"].mean
            )  # * only save the mean value of the fit result
    ...
    def main_gvar(self, data_dic_avg):
        ...
        for tseq in range(self.ra_tmin, self.ra_tmax):
            for tau in range(
                1 + self.tau_cut, tseq - self.tau_cut
            ):  # * because the tau in the data dic is from 1 to tseq - 1 without tseq, so tau_cut = 0 means tau from 1 to tseq - 1
                ra_t.append(tseq)
                ra_tau.append(tau)
...
def read_and_fit_bs(loop_paras):
    ...
    fit_id = "{}{}_P{}_L{}_b{}_z{}_tmax{}_cut{}".format(
        mass, gamma, mom, ll, b, z, ra_tmax, tau_cut
    )
    ...
    for tseq in range(4, 9):  # this tseq range comes from the raw data
        temp_ra_re, temp_ra_im = read_raw.read_ratio_bs(
            gamma, mass, mom, ll, b, z, tseq=tseq
        )
    ...
    gs_fit.para_set(
        pt2_tmin=3, pt2_tmax=9, ra_tmin=4, ra_tmax=ra_tmax, tau_cut=tau_cut
    )  #! here is the fitting parameter setting
...
def read_and_fit_gvar():
    ...
    for mass, gamma, mom in tqdm(
        itertools.product([220, 310], ["z", "t"], [8, 10, 12]), total=2 * 2 * 3
    ):
        ...
        ll = 6
        for b, z, tseq in itertools.product(range(1, 6), range(13), range(4, 9)):
            # for b in trange(1, 6, desc="b loop", leave=False):
            #     for z in trange(13, desc="z loop", leave=False):
            #         for tseq in range(4, 9):
            set_id = "{}{}_P{}_L{}_b{}_z{}_tseq{}".format(
                mass, gamma, mom, ll, b, z, tseq
            )

            temp_ra_re, temp_ra_im = read_raw.read_ratio_bs(
                gamma, mass, mom, ll, b, z, tseq
            )
    ...
    print(
        "\n>>> global average done. totally {} sets.".format(
            (len(data_dic_avg) - 24) / 5
        )
    )  # * should be 780
    ...
    for mass, gamma, mom in tqdm(
        ...
        for b in trange(1, 6, desc="b loop", leave=False):
            for z in trange(13, desc="z loop", leave=False):
                ...
                sub_data_dic_avg = {}
                sub_data_dic_avg["2pt_re"] = data_dic_avg[
                    "{}{}_P{}_2pt_re".format(mass, gamma, mom)
                ]
                sub_data_dic_avg["2pt_im"] = data_dic_avg[
                    "{}{}_P{}_2pt_im".format(mass, gamma, mom)
                ]
                ...
                for tseq in range(4, 9):
                    ...
                    sub_data_dic_avg["ra_re_tseq_{}".format(tseq)] = data_dic_avg[
                        set_id + "_ra_re"
                    ]
                    sub_data_dic_avg["ra_im_tseq_{}".format(tseq)] = data_dic_avg[
                        set_id + "_ra_im"
                    ]
                ...
                after_gs_fit[
                    "{}{}_P{}_L{}_b{}_z{}".format(mass, gamma, mom, ll, b, z)
                ] = collect
...
if __name__ == "__main__":
    ...
    gs_fit = Gs_Fit(
        two_state_fit(),
        fit_id="{}{}_P{}_L{}_b{}_z{}_tmax{}_cut{}".format(
            mass, gamma, mom, ll, b, z, tmax, tau_cut
        ),
    )
