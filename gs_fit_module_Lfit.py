...
class Gs_Fit():
    ...
    def main_gvar_chained(self, data_dic_avg):
        ...
        pt2_fit_res = lsf.nonlinear_fit(data=(x, y), prior=self.prior, fcn=self.get_fcn(fit_type='2pt'), maxit=10000, svdcut=1e-100, fitter='scipy_least_squares')
        ...
        #* chained prior
        chained_prior = self.prior
        chained_prior['log(dE1)'] = pt2_fit_res.p['log(dE1)']
        chained_prior['re_c1'] = pt2_fit_res.p['re_c1']


        fit_res = lsf.nonlinear_fit(data=(x, y), prior=chained_prior, fcn=self.get_fcn(fit_type='ra'), maxit=10000, svdcut=1e-100, fitter='scipy_least_squares')
    ...
    def get_fcn(self, fit_type=None):
...
def read_and_fit_gvar():
    ...
    for mass in tqdm([220, 310], desc='mass loop'):
        for gamma in tqdm(['z', 't'], desc='gamma loop'):
            for mom in tqdm([8, 10, 12], desc='mom loop'):
                ...
                for ll in [6, 8, 10]: #todo read all L 
    ...
    def constant_fit(data):
        ...
        Lfit_res = lsf.nonlinear_fit(data=(x, y), prior=priors, fcn=fcn, maxit=10000, svdcut=1e-100, fitter='scipy_least_squares')
        
        return Lfit_res.p['const']
                ...
                for b in trange(1, 6, desc='b loop', leave=False):
                    for z in trange(13, desc='z loop', leave=False):
                        ...
                        for ll in [10, 8, 6]:
                            ...
                            L_fit_re.append( fit_res.p['pdf_re'] )
                            L_fit_im.append( fit_res.p['pdf_im'] )
                        ...
                        collect['re'] = constant_fit(L_fit_re)
                        collect['im'] = constant_fit(L_fit_im)
    ...
    gv.dump(after_gs_fit, 'dump/gs_fit_gvar/all_after_gs_fit_2pt_tmin2_b45_cut0_Lfit.pkl')
