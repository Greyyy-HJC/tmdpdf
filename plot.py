...
'''
def hist_plot(dic, xlabel, title, xlim=None, ylim=None, accumulate=False, save=True):
...
def tmin_stability_plot(tmin_ls, gv_y_ls, Q_ls, logGBF_ls, title, chose_idx, save=True):
    ...
    axs[0].errorbar(tmin_ls, [v.mean for v in gv_y_ls], [v.sdev for v in gv_y_ls], **errorb)
    ...
    axs[0].set_ylabel(r'$\tilde{h}_{\Gamma}^{0}$', font)
    ...
    axs[2].set_xlabel(r'$t_{\rm min} / a$', font)
